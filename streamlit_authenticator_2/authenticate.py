...
class Authenticate:
    ...
    def _token_encode(self) -> str:
        ...
        if 'auth_extra_payload' in st.session_state:
            payload['auth_extra_payload'] = base64.b64encode(st.session_state['auth_extra_payload']).decode('utf-8')
    ...
    def _check_cookie(self):
        """
        Checks the validity of the reauthentication cookie and retrieves the auth_extra_payload.
        """
                ...
                if not st.session_state['logout']:
                    if self.token['exp_date'] > datetime.utcnow().timestamp():
                        if 'name' and 'username' in self.token:
                            ...
                            if 'auth_extra_payload' in self.token:
                                st.session_state['auth_extra_payload'] = base64.b64decode(self.token['auth_extra_payload'])
    ...
    def login(self, form_name: str, location: str = 'main', generate_extra: Callable[[str, str], bytes] | None = None) -> tuple:
        """
        ...
        if not st.session_state['authentication_status']:
            ...
            if not st.session_state['authentication_status']:
                ...
                # Check if generate_extra is provided and callable
                if generate_extra and callable(generate_extra):
                    extra_payload = generate_extra(self.username, self.password)
                    st.session_state['auth_extra_payload'] = extra_payload
