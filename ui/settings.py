...
def manage_models(selected_model_alias: str, model_repository: state.ModelRepository) -> dict:
    """
    ...
    model_selected_index = model_options.index(selected_model_alias) if selected_model_alias in model_options else 0
    selected_model_alias = st.selectbox('Select Model', model_options, index=model_selected_index)
    ...
    st.subheader(ADD_NEW_MODEL_TEXT if is_new_model else 'Environment Model Parameters' if is_env_model else 'Custom Model Settings')
    ...
    selected_model_alias = st.text_input('Model Alias (Display Name)', value=selected_model_alias if not is_new_model else '', disabled=is_env_model)
    is_openai_type = api_type == state.ApiTypeOptions.OPENAI.value

    model_or_deployment_name = st.text_input('Model Name (e.g. "gpt-3.5-turbo-0613")', value=selected_model.model_or_deployment_name if not is_new_model else '', disabled=is_env_model)
    api_key = st.text_input('API Key', selected_model.api_key if selected_model else '', disabled=is_env_model)
    api_version = st.text_input('API Version (e.g. "2023-07-01-preview")', selected_model.api_version if selected_model else '', disabled=is_env_model or is_openai_type)
    api_base = st.text_input('API Base (e.g. "https://my_model.openai.azure.com")', selected_model.api_base if selected_model else '', disabled=is_env_model or is_openai_type)
    temperature = st.slider(f'Temperature', 0.0, 1.0, 
                            selected_model.temperature if selected_model else 0.7, 0.01, 
                            disabled=is_env_model)
    ...
    if is_new_model:
        ...
        if not is_env_model:
            ...
            if st.button('Delete Model'):
                ...
                selected_model_alias = last_model.alias if last_model else None
                models_changed = True
    return {
