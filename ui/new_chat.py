...
def start_new_chat(
    ...
    with st.form("hidden"):
        ...
        if txt != st.session_state["nc_prompt_for_tokenizer"] and not (
            txt == "" and st.session_state["nc_prompt_for_tokenizer"] is None
        ):
            ...
            st.session_state["nc_chat_token_count"] = (
                None
                if st.session_state["nc_prompt_for_tokenizer"] is None
                else num_tokens_from_messages(
                    [{"role": "User", "content": txt}], tokenizer
                )
            )
    ...
    selected_model_alias = st.selectbox(
        "Model", model_options, key="select_model_dropdown", index=selected_model_index
    )
    ...
    button_title = (
        "Send"
        if st.session_state["nc_chat_token_count"] is None
    ...
    state_update = {"system_message": system_message, "temperature": temperature}
    if session:
